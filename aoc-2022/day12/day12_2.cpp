...
int main(int argc, char* argv[]) {
    ...
    q.emplace(0.0, endPos.first, endPos.second);
    ...
    while(!q.empty()) {
        ...
        for(int i = 0; i < 4; ++i) {
            ...
            if(isValid(a, ni, nj) && (a[ci][cj] - a[ni][nj]) <= 1) {
                if(!v[ni][nj]) {
    ...
    for(int i = 0; i < map.size(); ++i) {
        for(int j = 0; j < map[0].size(); ++j) {
            if(a[i][j] == 'a' && map[i][j].f != 0)
                vals.push_back(map[i][j].f);
        }
    }

    cout << *min_element(vals.begin(), vals.end());
}
